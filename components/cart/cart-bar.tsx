...
export function CartBar({
  ...
  return (
    <div className="fixed bottom-5 sm:bottom-6 left-3 sm:left-4 right-3 sm:right-4 z-40 max-w-lg sm:max-w-xl md:max-w-2xl mx-auto animate-in slide-in-from-bottom-12 duration-300">
      <button
        onClick={onOpenCart}
        className="w-full flex items-center justify-between p-3.5 sm:p-4 rounded-3xl bg-primary text-primary-foreground shadow-xl hover:shadow-primary/30 transition-all duration-300 hover:scale-[1.01] active:scale-98 relative overflow-hidden cursor-pointer"
      >
        ...
        <div className="flex items-center gap-3">
          <div className="relative bg-primary-foreground/15 p-2.5 rounded-2xl">
            <ShoppingBag className="h-5 w-5" />
            <span className="absolute -top-1 -right-1 bg-primary-foreground text-primary font-heading font-black text-[10px] w-5 h-5 rounded-full flex items-center justify-center border-2 border-primary animate-pulse">
              {totalItems}
            </span>
          </div>
