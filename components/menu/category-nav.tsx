    ...
    <div className="sticky top-0 z-30 w-full flex flex-col glass-panel border-b border-border shadow-md bg-card/95 backdrop-blur-md">
      ...
      <div className="flex items-center justify-between px-3.5 sm:px-4 py-2 sm:py-2.5 border-b border-border">
        ...
        <button
          ...
          title={lang === 'tr' ? "Konteyner Logosu (Büyütmek için tıklayın)" : "Konteyner Logo (Click to zoom)"}
        >
          <div className="transition-all duration-300 transform group-hover:scale-120 group-hover:shadow-md group-hover:shadow-foreground/15 group-active:scale-95 rounded-lg">
            <KonteynerLogo size="xs" shadow />
          </div>
          <div className="flex flex-col">
            <span className="font-black text-sm sm:text-base tracking-wider text-foreground group-hover:text-primary transition-colors">
              KONTEYNER
            </span>
            <span className="text-[9px] font-serif italic text-muted-foreground tracking-[0.2em] uppercase font-semibold -mt-1">
              Cafe &amp; Roastery
            </span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <Button
            ...
            className="h-8.5 w-8.5 sm:h-9 sm:w-9 rounded-full border border-border hover:bg-primary/10 cursor-pointer"
          >
        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 px-3.5 sm:px-4 py-1.5 sm:py-2 rounded-full text-[11px] sm:text-xs font-black uppercase tracking-wider transition-all duration-300 cursor-pointer border ${isActive
                ? "bg-primary text-primary-foreground border-primary shadow-md shadow-primary/25 scale-[1.02]"
                : "bg-secondary/70 dark:bg-card border-border hover:bg-secondary text-foreground/75 hover:text-foreground"
                }`}
            >
              {ad}
            </button>
